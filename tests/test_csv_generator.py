...
class TestCSVGenerator:
    ...
    # the below fixtures relate to the setup and teardown for testing data generation based on giving the
    ...
    def create_top_30_test_csv(self, generate_top_30_html_object):
        ...
        csv_generator_object.generate_top_30_csv(generate_top_30_html_object.get_top_30_table_elements_into_array(), TEST_RESOURCES_FOLDER, '/top_30_without_headers.data')
        yield self.create_top_30_test_csv
        os.remove(TEST_RESOURCES_FOLDER + '/top_30_without_headers.data')

    @pytest.fixture(scope='module', autouse=True)
    def create_top_30_test_csv_with_headers_with_specified_location(self, generate_top_30_html_object):
        csv_generator_object = Top30CSVGenerator()
        csv_generator_object.generate_top_30_csv(generate_top_30_html_object.get_top_30_table_elements_into_array(), TEST_RESOURCES_FOLDER, '/top_30_with_headers.data', 'yes')
        yield self.create_top_30_test_csv_with_headers_with_specified_location
        os.remove(TEST_RESOURCES_FOLDER + '/top_30_with_headers.data')
    ...
    def create_top_30_using_default_location_and_default_name(self, generate_top_30_html_object):
        ...
        os.remove(os.getcwd().replace("\\","/") + '/Flask/Downloads/' + 'ItJobsWatchTop30.data')

    # testing data generation based on giving the top 30 CSV Generator a defined path
    def test_csv_without_headers(self):
        list_count = 0
        with open(TEST_RESOURCES_FOLDER + '/top_30_without_headers.data') as top_30_without_headers:
    ...
    def test_csv_with_headers(self):
        list_count = 0
        with open(TEST_RESOURCES_FOLDER + '/top_30_with_headers.data') as top_30_without_headers:
            parsed_csv_file = csv.reader(top_30_without_headers)
            for line in parsed_csv_file:
                list_count += 1
    ...
    def test_csv_without_headers_default_location(self):
        list_count = 0
        with open(os.getcwd().replace("\\","/") + '/Flask/Downloads/' + 'ItJobsWatchTop30.data') as top_30_without_headers:
            parsed_csv_file = csv.reader(top_30_without_headers)
            for line in parsed_csv_file:
                list_count += 1
